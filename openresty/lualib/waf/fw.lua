...
local function _parse_collection(self, collection, opts,rule)
	local lookup = {
...
local function _parse_dynamic_value(self, key, collections)
	...
	local str = ngx.re.gsub(key, [=[%{([^{]*)}]=], lookup, 'osij')
...
local function _process_rule(self, rule, collections, ctx ,is)
	local id = rule.id
	local var = rule.var
	...
	if (type(collections[var.type]) == "function") then -- dynamic collection data - pers. storage, score, etc
		...
		if (not ctx.collections_key[memokey]) then
			...
			t = _parse_collection(self, collections[var.type], var.opts,rule)
...
local function high_wl_boundary(self,t,ctx)
	
	if not self._active_highwl then
		return false
	end
	for _, ruleset in ipairs(self._active_highwl) do
		_log(self, "Beginning ruleset " .. ruleset)
		_log(self, "Requiring " .. ruleset)
		local rs = require("waf.rules.cms." .. ruleset)

		for __, rule in ipairs(rs.rules()) do
			
			if rule.var.type == "BOUNDARY_NAME" then
				local uri_match = operators[rule.var.uri_operator](self, ngx.var.uri, rule.var.uri_pattern, ctx)
				if uri_match then
                    if rule.var.pattern == '0' then
                         t = nil 
                         return true
                    end
					if t then
						local match = operators[rule.var.operator](self, t, rule.var.pattern, ctx)
						if match then
							return true
						end
					end
...
local function _parse_request_body(self, ctx ,request_headers, is_boundary)
	...
	if (ngx.re.find(content_type_header, [=[^multipart/form-data; boundary=]=], self._pcre_flags)) then
		...
		local lasttype, chunk, from ,to ,err --,boundary_name ,boundary_filename ,boundary_value 
		local boundary_name = {}
		local boundary_filename = {} 
		local boundary_value = {}
		local match = false
		
		local collection_boundary = {
			BOUNDARY_NAME = function() return boundary_name end,
			BOUNDARY_VALUE = function() return boundary_value end,
			BOUNDARY_FILENAME = function() return boundary_filename end,
		}
		...
		while true do
			...
			if (typ == "header") then
				...
				from,to,err = ngx.re.find(chunk, "name=\"(.+?)\"","isjo",nil,1)
				if from then
					match = high_wl_boundary(self,string.sub(chunk, from, to),ctx)
					if not match then
						table.insert(boundary_name,string.sub(chunk, from, to))
					end
				end
				from,to,err = ngx.re.find(chunk, "filename=\"(.+?)\"","sijo",nil,1)
				if from then
					table.insert(boundary_filename,string.sub(chunk, from, to))
				end
				ngx.req.append_body("\n" .. chunk)
			elseif (typ == "body") then
				...
				if (lasttype == "header") then
					if not match then
						if #chunk > 2048 then
							table.insert(boundary_value,string.sub(chunk, 1, 2048))
						else
							table.insert(boundary_value,chunk)
						end
					end
...
local function high_wl(self,collections,ctx)
	...
	for _, ruleset in ipairs(self._active_highwl) do
		...
		for __, rule in ipairs(rs.rules()) do
			local uri_match = operators[rule.var.uri_operator](self, ngx.var.request_uri, rule.var.uri_pattern, ctx)
			if (uri_match) then
                local t = collections[rule.var.type]
                if rule.var.pattern == '0' then
                    t = nil 
                    return
                end
				if t and type(t) == 'table' then
					for k , v in pairs(t) do
						local match = operators[rule.var.operator](self, k, rule.var.pattern, ctx)
						if (match) then
							t[k] = nil
						end
					end 
...
function _M.exec(self)
	...
	local request_post_args = _parse_request_body(self, nil,request_headers,false)
	...
	if self._high_rule then
		_parse_request_body(self, ctx ,request_headers,true)
	end
end -- fw.exec()
...
function _M.new(self)
	return setmetatable({
		...
		_whitelist = {},
		_blacklist = {},
		_active_rulesets = waf.rulesets, 
		_active_highwl = {},
		_ignored_rules = waf.ignored_rules,
		_allowed_content_types = waf.content_type,
		_debug = false,
		...
		_pcre_flags = 'oisj',
		_score_threshold = waf.score_threshold,
		_high_rule = false,
		_storage_zone = nil
	}, mt)
end
...
function _M.set_option(self, option, value)
	local lookup = {
		...
		active_highwl = function(value)
			local t = self._active_highwl
			self._active_highwl[#t + 1] = value
		end,
